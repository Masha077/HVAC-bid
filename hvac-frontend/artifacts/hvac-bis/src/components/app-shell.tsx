...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const activeProjectName = activeResult?.unified_project?.building_type
    ? `${activeResult.unified_project.building_type} HVAC`
    : activeResult?.project_id
    ? `Project ${activeResult.project_id}`
    : 'Active Workspace';

  const activeProjectLocation = activeResult?.unified_project?.location || 'Live Engineering Session';

  const displayName = profile?.full_name || user?.email?.split('@')[0] || 'MEP Engineer';
  const displayCompany = profile?.company_name || 'MEP Engineering';
        ...
        <header className="sticky top-0 z-20 flex h-[78px] items-center gap-3 border-b border-border bg-background/95 px-4 backdrop-blur-md sm:px-7">
          ...
          <div className="min-w-0 flex-1">
            <button className="group flex items-center gap-2 text-left" data-testid="button-project-selector">
              ...
              <span className="max-w-[190px] truncate text-sm font-semibold text-foreground sm:max-w-none">{activeProjectName}</span>
            ...
            <div className="mt-1 hidden items-center gap-1.5 text-[11px] text-muted-foreground sm:flex">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500"/>{activeProjectLocation} <span className="mx-1 text-border">·</span> LIVE BACKEND
            </div>
          </div>
    ...
    <div className="fixed right-3 top-[84px] z-30 w-[min(360px,calc(100vw-24px))] rounded-2xl border border-border bg-card p-4 shadow-2xl animate-rise">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="mt-1 text-base font-semibold">Latest project runs</h3>
        </div>
      ...
      <div className="mt-4 space-y-1">
        {loading ? (
          ...
          <div className="p-4 text-center text-xs text-muted-foreground">No historical project runs found.</div>
        ) : (
          historyItems.slice(0,5).map(item => (
            <Link key={item.id} href={`/projects/${item.id}`} onClick={onClose} className="flex items-center gap-3 rounded-lg p-2.5 hover:bg-muted" data-testid={`history-project-${item.id}`}>
              <div className="rounded-md bg-[#f5e4dc] p-2 text-primary">
                <FileText size={14}/>
              </div>
              <div className="min-w-0 flex-1">
                <div className="truncate text-xs font-medium">{item.name || item.id}</div>
                <div className="mt-0.5 text-[10px] text-muted-foreground">{item.buildingType || 'HVAC Project'} · {item.updatedAt}</div>
              </div>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${item.status === 'SUCCESS' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                {item.status || 'SUCCESS'}
              </span>
            </Link>
          ))
